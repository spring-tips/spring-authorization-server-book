...
Generate a key pair.
You can use `ssh-keygen` to create the keys, like this.
...
[source,properties]
----
jwt.key.id=configured-jwt-key-1234567890
...
[source,java]
----
include::authorization-server/src/main/java/bootiful/authorizationserver/KeyConfiguration.java[]
----

In this example, I've kept the private key in the source code of this repository, but as you can imagine, this is a very bad idea in production.
Don't forget my admonition earlier: take care to handle your credentials securely.
Perhaps in something like Hashicorp Vault!
