...
It is the first port-of-call for all requests intended for our system.
When visitors to our site punch in the domain name for our system into a browser, it'll resolve to a loadbalancer serving this gateway service.
This is where the users of our system will come and so it is here we'll want to originate an OAuth token in the browser.
...
There's very little Java code in the gateway module.
...
We'll configure two Spring Cloud Gateway _routes_. Each request has a predicate, optional filter(s), and a destination URI. The predicate defines how requests to the Spring Cloud Gateway service are matched, e.g.: does this request have a particular header or cookie, or a particlar path, or a particular virtual host? You may specify one or more filters that act on the incoming requests, changing it. Finally, the request, after it has passed through any and all filters, is sent to a final destination, which we specify with a URI.
