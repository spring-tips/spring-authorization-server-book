    ...
    ApplicationListener<ApplicationReadyEvent> applicationReadyListener(
        ...
        return event -> {
            if (repository.findKeyPairs().isEmpty())
                publisher.publishEvent(new KeyPairGenerationRequestEvent(Instant.now()));
        };
    }

    @Bean
    JdbcRsaKeyPairRepository jdbcRsaKeyPairRepository(
            RowMapper<RsaKeyPairRepository.RsaKeyPair> keyPairRowMapper,
            JdbcTemplate template, KeySerde serde) {
        return new JdbcRsaKeyPairRepository(keyPairRowMapper, template, serde);
    }

    @Bean
    RsaKeyPairRowMapper keyPairRowMapper(KeySerde serde) {
...
class RsaKeyPairRowMapper implements RowMapper<RsaKeyPairRepository.RsaKeyPair> {
    ...
    private final KeySerde serde;

    RsaKeyPairRowMapper(KeySerde serde) {
        this.serde = serde;
    }
    ...
    public RsaKeyPairRepository.RsaKeyPair mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        return new RsaKeyPairRepository.RsaKeyPair(
                rs.getString("id"),
...
class JdbcRsaKeyPairRepository implements RsaKeyPairRepository {
    ...
    JdbcRsaKeyPairRepository(RowMapper<RsaKeyPair> keyPairRowMapper, JdbcTemplate template, KeySerde keySerde) {
